...
\begin{frame}[fragile]
  \frametitle{思考}
	...
	\begin{minted}[bgcolor=LightGray, baselinestretch=.9, fontsize=\small]{sql}
-- 下面的查询是否正确？
SELECT section.course_id, section.sec_id, COUNT(takes.id)
FROM takes, section
WHERE takes.year = section.year
        AND takes.semester = section.semester
        AND takes.course_id = section.course_id
...
\begin{frame}[fragile]
	\begin{minted}[bgcolor=LightGray, baselinestretch=.9, fontsize=\small]{sql}
 WITH section_tmp(course_id, sec_id, semester, year) AS (
    VALUES
        ('CS101', '1', 'Fall', 2017),   -- 有人选
        ('CS102', '1', 'Fall', 2017)    -- 无人选 ← 关键
),
takes_tmp(id, course_id, sec_id, semester, year) AS (
    VALUES
        ('s001', 'CS101', '1', 'Fall', 2017)
)
SELECT s.course_id, s.sec_id, COUNT(t.id) AS student_count
FROM takes_tmp t, section_tmp s
WHERE t.year      = s.year
