...
\begin{frame}
	...
	\begin{tikzpicture}
		\colorlet{good}{green!75!black}    % 期末测试
		\colorlet{bad}{red}                % 作业
		\colorlet{neutral}{orange!80}      % 期末自评 (新增)
		\colorlet{none}{blue!75!black}     % 期末Project

		\node[align=center,text width=3cm]{\textbf{考核权重}};

		\begin{scope}[line width=4mm,rotate=270]
			% 1. 期末测试 (30%): 0 到 108 度
			\draw[good] (0:2cm) arc (0:108:2cm);

			% 2. 作业 (30%): 108 到 216 度
			\draw[bad] (108:2cm) arc (108:216:2cm);

			% 3. 期末Project (20%): 216 到 288 度
			\draw[none] (216:2cm) arc (216:288:2cm);

			% 4. 期末自评 (20%): 288 到 360 度
			\draw[neutral] (288:2cm) arc (288:360:2cm);

			% 刻度线 (每 7.2 度一个刻度，共 50 个刻度表示 100%)
			\newcount\mycount
			...
			% 文字标注 (根据旋转后的坐标手动微调位置)
			\draw (54:2.3cm)  node[right] {期末测试 (30\%)};
			\draw (165:2.5cm) node[right]  {作业 (30\%)};
			\draw (252:2.3cm) node[left]  {期末Project (20\%)};
			\draw (324:2.3cm) node[left] {期末自评 (20\%)};
		\end{scope}
	\end{tikzpicture}
	\begin{tikzpicture}
