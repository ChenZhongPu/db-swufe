...
\begin{frame}
	...
	\begin{block}{重点}
		\begin{itemize}
			\item (对于CS专业) 理论和应用同等重要
			\item  （对于非CS专业）主要是如何应用数据库
		\end{itemize}
...
\begin{frame}
	...
	\begin{tikzpicture}
		\colorlet{self}{orange!80}        % 期末自评 20%
		\colorlet{project}{blue!75!black} % 期末Project 20%
		\colorlet{homework}{red}          % 作业 15%
		\colorlet{quiz}{green!75!black}   % 随堂测试 40%
		\colorlet{attendance}{purple}     % 考勤 5%

		\node[align=center,text width=3cm]{\textbf{考核权重}};

		\begin{scope}[line width=4mm,rotate=270]

			% 1. 期末自评 (20%) 0 → 72
			\draw[self] (0:2cm) arc (0:72:2cm);

			% 2. 期末Project (20%) 72 → 144
			\draw[project] (72:2cm) arc (72:144:2cm);

			% 3. 作业 (15%) 144 → 198
			\draw[homework] (144:2cm) arc (144:198:2cm);

			% 4. 随堂测试 (40%) 198 → 342
			\draw[quiz] (198:2cm) arc (198:342:2cm);

			% 5. 考勤 (5%) 342 → 360
			\draw[attendance] (342:2cm) arc (342:360:2cm);

			% 刻度线 (每 7.2 度 = 2%)
			\foreach \angle in {0,7.2,...,359}
				{
					\draw[black!15,thick] (\angle:18mm) -- (\angle:22mm);
				}

			% 文字标注（取各区间中点）
			\draw (36:2.3cm)  node[right] {期末自评 (20\%)};
			\draw (108:2.3cm) node[right] {期末Project (20\%)};
			\draw (171:2.5cm) node[right] {作业 (15\%)};
			\draw (270:2.5cm) node[left]  {随堂测试 (40\%)};
			\draw (351:2.3cm) node[left]  {考勤 (5\%)};
...
\end{frame}
\begin{frame}
	\frametitle{软件工具：PostgreSQL}
	PostgreSQL 18: \emph{The World's Most Advanced Open Source Relational Database}
...
\begin{frame}
	...
	\href{https://openai.com/index/scaling-postgresql/}{Scaling PostgreSQL to power 800 million ChatGPT users}
\end{frame}

\begin{frame}
	\frametitle{软件工具：DataGrip}
...
\begin{frame}
	\frametitle{软件工具：DuckDB}
	\href{https://duckdb.org/}{DuckDB}，嵌入式分析型数据库，适合学习数据库应用。

	\begin{center}
		\includegraphics[width=.55\paperwidth]{image/duckdb}
	\end{center}

\end{frame}

\begin{frame}
	\frametitle{课程资源：GitHub}

	{\large \url{github.com/ChenZhongPu/db-swufe}
	}
\end{frame}
...
\begin{frame}
	\frametitle{警惕AI时代下的BI}

	{\tiny 参考 \url{https://wiki.csie.ncku.edu.tw/linux} }

	\begin{block}{作家还是速记员？}
		1866年，陀思妥耶夫斯基与出版商签下苛刻合约，必须在11月1日前交出一部长篇小说，否则未来九年的作品版权将被夺走；当时他正写《罪与罚》，债务缠身且癫痫频发，时间所剩无几。年轻速记员 Anna Grigoryevna Snitkina 受聘协助，他口述，她速记整理，26天内完成《赌徒》，如期交稿。小说次年出版，两人亦于1867年结婚，这段合作最终改变了他的人生轨迹。
	\end{block}
