...
\begin{frame}[fragile]
  \frametitle{Multi-Model DBMSs}
  在\url{https://db-engines.com/en/ranking}中，主流数据库的分类都是：\textbf{Relational，Multi-Model}。因此，它们支持非关系的存储和操作，并不一定要求1NF。

\begin{minted}[bgcolor=LightGray, baselinestretch=1]{sql}
-- DuckDB 
CREATE TABLE example (j JSON);
INSERT INTO example VALUES('{"name": "Mike", "age": 20, 
            "hobbies": ["Baskeball", "Sing", "Dance"]}');
SELECT j->'$.name' FROM example;
\end{minted}

\small{\url{https://duckdb.org/docs/current/data/json/json_functions}}

\end{frame}

\begin{frame}[fragile]
  \frametitle{练习}
  {\small 参考 \url{https://www.postgresql.org/docs/current/datatype-json.html}，在PostgreSQL中实现：}
...
-- INSERT，包括name、title、skills
SELECT attributes->>'name' FROM employees WHERE id = 1; 
SELECT attributes->'skills' FROM employees WHERE id = 1; 
SELECT attributes->>'Name' FROM employees
WHERE attributes->'skills' @> '["Python"]';
\end{minted}
\end{frame}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item \textbf{超码}：当属性集是一个超码，使用$K$表示。我们可以说“K是R的一个超码”。
	\end{itemize}
...
\begin{frame}
  \frametitle{练习}
假设将R=(A, B, C, D, E)被分解成(A, B, C)和(A, D, E)。如果下面的函数依赖成立，请说明该分解是无损的。
...
\begin{frame}
  \frametitle{闭包}
定义：给定一个集合 $S$ 和其上的某种运算 $\circ$。如果对于 $S$ 中的任意元素 $a$ 和 $b$，经过该运算后的结果 $a \circ b$ 仍然属于集合 $S$，我们就说集合 $S$ 对运算 $\circ$ 是封闭的。

通俗理解：在这个集合里做运算，结果永远“跑不出”这个集合的边界。
\end{frame}

\begin{frame}
  \frametitle{阿姆斯特朗公理}
  应用\alert{阿姆斯特朗公理（Armstrong's axioms）}可以找到闭包$F^+$：
  \begin{itemize}
    \item \alert{自反律}（Reflexivity rule）：如果 $\alpha$ 是一个属性集且 $\beta \subseteq \alpha$，那么 $\alpha \rightarrow \beta$ 成立。
    \item \alert{增广律}（Augmentation rule）：如果 $\alpha \rightarrow \beta$ 成立且 $\gamma$ 是一个属性集，那么 $\gamma\alpha \rightarrow \gamma\beta$ 成立。
    \item \alert{传递律}（Transitivity rule）：如果 $\alpha \rightarrow \beta$ 成立且 $\beta \rightarrow \gamma$ 成立，那么 $\alpha \rightarrow \gamma$ 成立。
\end{itemize}
\end{frame}
...
\begin{itemize}
\item 如果 $\alpha \rightarrow \beta$ 成立且 $\alpha \rightarrow \gamma$ 成立，那么 $\alpha \rightarrow \beta\gamma$ 成立。
\item 如果 $\alpha \rightarrow \beta\gamma$ 成立，那么 $\alpha \rightarrow \beta$ 成立且 $\alpha \rightarrow \gamma$ 成立。
\item 如果 $\alpha \rightarrow \beta$ 成立且 $\gamma\beta \rightarrow \delta$ 成立，那么 $\alpha\gamma \rightarrow \delta$ 成立。
\end{itemize}
\end{frame}


\begin{frame}
  \frametitle{基于阿姆斯特朗公理的推论}

\begin{itemize}
  \item \alert{合并律}（Union rule）：如果 $\alpha \rightarrow \beta$ 成立且 $\alpha \rightarrow \gamma$ 成立，那么 $\alpha \rightarrow \beta\gamma$ 成立。
  \item \alert{分解律}（Decomposition rule）：如果 $\alpha \rightarrow \beta\gamma$ 成立，那么 $\alpha \rightarrow \beta$ 成立且 $\alpha \rightarrow \gamma$ 成立。
  \item \alert{伪传递律}（Pseudotransitivity rule）：如果 $\alpha \rightarrow \beta$ 成立且 $\gamma\beta \rightarrow \delta$ 成立，那么 $\alpha\gamma \rightarrow \delta$ 成立。
\end{itemize}
\end{frame}
